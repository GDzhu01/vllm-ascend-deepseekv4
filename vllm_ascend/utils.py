...
def get_compressed_pos_and_indices(
    ...
    assert np.all(num_computed_tokens >= 0) and np.all(num_scheduled_tokens >= 0), "Token count cannot be negative value"
    ...
    for compress_ratio in compress_ratios:
        ...
        compressed_pos_ids = np.arange(np.sum(num_new_compressed_pos)) + np.repeat(pos_starts - prefix_offsets, num_new_compressed_pos)
