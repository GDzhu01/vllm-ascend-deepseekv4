...
class AscendDSAMetadataBuilder(AttentionMetadataBuilder[AscendDSAMetadata]):
    ...
    def __init__(
        ...
        if AscendDSAMetadataBuilder.decode_sas_c1_metadata is None:
            AscendDSAMetadataBuilder.decode_sas_c1_metadata = torch.zeros(1024, dtype=torch.int32, device=self.device)
...
class AscendDSAImpl(DSAAttentionImpl):
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        # transpose weight for transpose batch matmul op
        self.wo_a.weight.data = self.wo_a.weight.data.view(self.n_local_groups, self.o_lora_rank, -1).transpose(2,1).contiguous()
    ...
    def forward(
        ...
        # replace torch.einsum with npu_transpose_batchmatmul
