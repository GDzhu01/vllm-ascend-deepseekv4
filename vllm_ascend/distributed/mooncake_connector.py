...
class KVCacheRecvingThread(threading.Thread):
    ...
    def __init__(self, tp_rank: int, tp_size: int, _prefill_pp_size: int,
                 ...
                 local_handshake_port: int, use_mla: bool, use_sparse: bool, is_dsv4: bool,
                 local_kv_caches_base_addr: list[int], block_len: list[int], state_addr_start_idx: int,
    ...
    def add_request(self, request_id: str, local_block_ids: list[int], local_state_id: int,
                    remote_block_ids: list[int], remote_state_id: int, remote_engine_id: str,
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]):
        ...
        if self._prefill_pp_size > 1:
            ...
            if self.is_dsv4:
                remote_state_base_addrs = remote_kv_caches_base_addrs[self.state_addr_start_idx: ]
                remote_kv_caches_base_addrs = remote_kv_caches_base_addrs[: self.state_addr_start_idx]
                local_state_base_addrs = local_kv_caches_base_addrs[self.state_addr_start_idx: ]
                local_kv_caches_base_addrs = local_kv_caches_base_addrs[: self.state_addr_start_idx]
        ...
        if self.is_dsv4:
            for k, (src_state_base_addr, dst_state_base_addr) in enumerate(
                zip(local_state_base_addrs, remote_state_base_addrs)):
                block_len = self.block_len[self.state_addr_start_idx + k]
                src_list.append(src_state_base_addr + block_len * local_state_id)
                dst_list.append(dst_state_base_addr + block_len * remote_state_id)
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor], state_caches: dict[str, tuple[torch.Tensor]]):
        ...
        for layer_name, kv_cache_tuple in kv_caches.items():
            for single_kv_cache in kv_cache_tuple:
                ...
                self.block_len.append(single_kv_cache.element_size() * math.prod(block_shape))
                self.kv_caches_base_addr.append(single_kv_cache.data_ptr())
                lengths.append(self.num_blocks * self.block_len[-1])
                logger.info(
                    "layer: %s, num_blocks: %s, block_shape: %s",
                    layer_name, self.num_blocks, block_shape)
        
        self.state_addr_start_idx = len(self.kv_caches_base_addr)
        if self.is_dsv4:
            ...
            for layer_name, state_cache_tuple in state_caches.items():
                for single_state_cache in state_cache_tuple:
                    ...
                    self.block_len.append(single_state_cache.element_size() * math.prod(block_shape))
                    self.kv_caches_base_addr.append(single_state_cache.data_ptr())
                    lengths.append(self.num_state_blocks * self.block_len[-1])
                    logger.info(
                        "layer: %s, num_state_blocks: %s, block_shape: %s",
                        layer_name, self.num_state_blocks, block_shape)

        global_te.register_buffer(self.kv_caches_base_addr, lengths)
        ...
        if self.kv_role == 'kv_producer':
            ...
            self.kv_recv_thread = KVCacheRecvingThread(
                ...
                self.engine_id, self.handshake_port, self.use_mla, self.use_sparse, self.is_dsv4, self.kv_caches_base_addr,
                self.block_len, self.state_addr_start_idx, ready_event, self.vllm_config, self.kv_caches)
