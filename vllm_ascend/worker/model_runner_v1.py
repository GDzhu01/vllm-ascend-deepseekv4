...
from vllm_ascend.compilation.unified_mtp_graph import (
    UnifiedMTPACLGraphWrapper,
)
...
class NPUModelRunner(GPUModelRunner):
    ...
    def _set_up_drafter(self):
        ...
        self.unified_mtp_graph: UnifiedMTPACLGraphWrapper | None = None
        self.unified_mtp_runner: UnifiedMTPGraphRunnerSafe | None = None
        ...
        if self.speculative_config:
            ...
            if get_pp_group().is_last_rank:
                ...
                if self.speculative_config.method == "mtp" and self._use_aclgraph():
                    self._init_unified_mtp_graph()
        self.discard_request_indices = self._make_buffer(self.max_num_reqs, dtype=torch.int64)
        self.num_discarded_requests = 0
    
    def _init_unified_mtp_graph(self):
        """Build the unified MTP ACLGraph wrapper if drafter supports it.

        The wrapper hosts a :class:`UnifiedMTPGraphRunnerSafe` (validation +
        fallback) and is captured/replayed by the standard ACLGraph
        machinery, so the main + MTP forward share a single NPU stream and
        skip the legacy two-graph stream sync.
        """
        ...
        try:
            ...
            self.unified_mtp_runner = UnifiedMTPGraphRunnerSafe(
                main_model=self.model,
                draft_model=draft_model,
                vllm_config=self.vllm_config,
                device=self.device,
            )
            # Match the wrapper runtime_mode to the configured cudagraph
            # mode. ACLGraphWrapper.__call__ only triggers capture/replay
            # when ``forward_context.cudagraph_runtime_mode == runtime_mode``,
            # so picking the wrong mode here silently disables the unified
            # graph path on FULL_DECODE_ONLY deployments.
            cudagraph_mode_cfg = self.compilation_config.cudagraph_mode
            if cudagraph_mode_cfg == CUDAGraphMode.FULL_DECODE_ONLY:
                wrapper_runtime_mode = CUDAGraphMode.FULL_DECODE_ONLY
            else:
                wrapper_runtime_mode = CUDAGraphMode.FULL
            self.unified_mtp_graph = UnifiedMTPACLGraphWrapper(
                unified_runner=self.unified_mtp_runner,
                vllm_config=self.vllm_config,
                runtime_mode=wrapper_runtime_mode,
            )
            if hasattr(self.drafter, 'set_enable_unified_graph'):
                self.drafter.set_enable_unified_graph(True)
        except Exception as e:
            logger.warning(f"Failed to initialize unified MTP graph: {e}")
            self.unified_mtp_graph = None
            self.unified_mtp_runner = None
    
    def _prepare_draft_attn_metadata_for_unified_graph(
        self,
        ...
        if self.drafter is None:
            return None
        if not hasattr(self.drafter, 'prepare_inputs_padded'):
            return None
        try:
            batch_size = num_tokens // (self.num_spec_tokens + 1)
            common_attn_metadata, _ = self.drafter.prepare_inputs_padded(
            ...
            logger.warning(f"Failed to prepare draft attn metadata: {e}")
            return None
    
    def _get_draft_inputs_for_unified_graph(
        self,
        hidden_states: torch.Tensor | None,
        positions: torch.Tensor,
        num_tokens: int,
    ) -> dict | None:
        """Return tensors that should be fed to the draft model under the
        unified graph.

        Whenever possible we reuse the drafter's pre-allocated buffers
        (``self.drafter.hidden_states`` / ``positions`` / ``input_ids``) so
        that the address fed into ACLGraph capture matches the address used
        on every replay. Slicing a fresh ``hidden_states`` tensor on every
        step would change the address and trigger a recapture.
        """
        if self.drafter is None:
            return None
        if hasattr(self.drafter, 'get_draft_inputs_for_unified_graph'):
            return self.drafter.get_draft_inputs_for_unified_graph(
                hidden_states, positions, num_tokens
            )
        draft_hidden_states = None
        if hasattr(self.drafter, 'hidden_states'):
            draft_hidden_states = self.drafter.hidden_states[:num_tokens]
        elif hidden_states is not None:
            draft_hidden_states = hidden_states[:num_tokens]
        if hasattr(self.drafter, 'positions'):
            draft_positions = self.drafter.positions[:num_tokens]
        else:
            draft_positions = positions[:num_tokens]
        draft_input_ids = None
        if hasattr(self.drafter, 'input_ids'):
            draft_input_ids = self.drafter.input_ids[:num_tokens]
        return {
    ...
    def propose_draft_token_ids(
        ...
        unified_draft_token_ids = getattr(
            self, "_draft_token_ids_from_unified_graph", None
        )
        if unified_draft_token_ids is not None:
            self._draft_token_ids_from_unified_graph = None
            return unified_draft_token_ids
        if not self.drafter:
    ...
    def execute_model(
        ...
        with (
            ...
            # FULL_DECODE_ONLY also dispatches as a "full" graph at runtime
            # (see has_full_cudagraphs()); both modes share the same wrapper.
            if self.unified_mtp_graph is not None and cudagraph_mode in (
                CUDAGraphMode.FULL,
                CUDAGraphMode.FULL_DECODE_ONLY,
            ):
                if use_spec_decode and self.speculative_config is not None:
                    if self.speculative_config.method == "mtp":
                        # NOTE: ``get_pp_group`` is imported at module level
                        # (see top-of-file ``from vllm.distributed.parallel_state
                        # import ...``). Re-importing it here would shadow the
                        # module-level binding and turn ``get_pp_group`` into a
                        # function-local that is unbound on every other code
                        # path in ``execute_model`` (UnboundLocalError).
                        can_use, reason = should_use_unified_graph(
                            ...
                            use_compress=self.use_compress,
                            is_pp_last_rank=get_pp_group().is_last_rank,
                        )
                        use_unified_graph = can_use
                        if not can_use:
                            logger.debug(f"Unified graph disabled: {reason}")
            if use_unified_graph:
                draft_token_indices_to_sample = None
                self._draft_token_ids_from_unified_graph = None
                
                batch_size = scheduler_output.total_num_scheduled_tokens // (self.num_spec_tokens + 1)
                if hasattr(self.drafter, 'token_indices_to_sample'):
                    sample_count = batch_size * self.drafter.extra_slots_per_request
                    draft_token_indices_to_sample = (
                        self.drafter.token_indices_to_sample[:sample_count]
                    )
                
                draft_attn_metadata_for_graph = self._prepare_draft_attn_metadata_for_unified_graph(
                    spec_decode_common_attn_metadata,
                    scheduler_output.total_num_scheduled_tokens,
                    scheduler_output,
                )
                
                draft_inputs = self._get_draft_inputs_for_unified_graph(
                    None,
                    positions,
                    scheduler_output.total_num_scheduled_tokens,
                )
                draft_hidden_states = draft_inputs.get('draft_hidden_states') if draft_inputs else None
                draft_positions = draft_inputs.get('draft_positions') if draft_inputs else None
                
                try:
                    unified_output = self._model_forward_unified_mtp(
                        num_tokens_padded,
                        input_ids,
                        positions,
                        intermediate_tensors,
                        ...
                        logits_indices,
                        scheduler_output.total_num_scheduled_tokens,
                        draft_token_indices_to_sample=draft_token_indices_to_sample,
                        draft_attn_metadata=draft_attn_metadata_for_graph,
                        draft_hidden_states=draft_hidden_states,
                        draft_positions=draft_positions,
                        **model_kwargs,
                    )
                except UnifiedMTPGraphError as e:
                    logger.warning(f"Unified MTP graph failed, falling back: {e}")
                    if self.unified_mtp_runner is not None:
                        self.unified_mtp_runner.reset_error_state()
                    unified_output = UnifiedGraphOutput(
                        hidden_states=None,
                        fallback_used=True,
                        error=str(e),
                    )
                
                if isinstance(unified_output, UnifiedGraphOutput):
                    if unified_output.fallback_used or unified_output.hidden_states is None:
                        hidden_states = self._model_forward(
                            num_tokens_padded, input_ids, positions, intermediate_tensors, inputs_embeds, **model_kwargs
                        )
                    else:
                        self._draft_token_ids_from_unified_graph = unified_output.draft_token_ids
                        hidden_states = unified_output.hidden_states
                        # If the unified runner produced aux_hidden_states,
                        # fold them back into the tuple shape that the
                        # post-process block below expects when
                        # ``use_aux_hidden_state_outputs`` is set.
                        if (
                            self.use_aux_hidden_state_outputs
                            and unified_output.aux_hidden_states is not None
                        ):
                            hidden_states = (
                                hidden_states,
                                unified_output.aux_hidden_states,
                            )
                else:
                    hidden_states = unified_output
            else:
                hidden_states = self._model_forward(
                    num_tokens_padded, input_ids, positions, intermediate_tensors, inputs_embeds, **model_kwargs
                )
    ...
    def _model_forward_unified_mtp(
        self,
        num_tokens_padded: int,
        ...
        **model_kwargs,
        ...
        if self.unified_mtp_graph is None:
            return UnifiedGraphOutput(
                hidden_states=self._model_forward(
                    num_tokens_padded, input_ids, positions, intermediate_tensors, inputs_embeds, **model_kwargs
                ),
                fallback_used=True,
                error="unified_mtp_graph not initialized",
            )

        logger.info_once(
            "Unified main + MTP ACLGraph path is active "
        ...
        if self.drafter is not None:
            if hasattr(self.drafter, 'input_ids'):
                ...
                draft_input_ids = input_ids[:num_tokens]

        if draft_hidden_states is None and self.drafter is not None:
            if hasattr(self.drafter, 'hidden_states'):
                draft_hidden_states = self.drafter.hidden_states[:num_tokens]

        if draft_positions is None:
            if self.drafter is not None and hasattr(self.drafter, 'positions'):
                draft_positions = self.drafter.positions[:num_tokens]
            else:
                draft_positions = positions[:num_tokens]

        # Route through the ACLGraphWrapper so the unified forward is captured
        # and replayed as a single graph. The wrapper falls back to direct
        # call when ``aclgraph_runtime_mode`` is NONE (e.g. profile run).
        try:
            output = self.unified_mtp_graph(
                input_ids=input_ids,
                positions=positions,
                intermediate_tensors=intermediate_tensors,
                inputs_embeds=inputs_embeds,
                ...
                draft_hidden_states=draft_hidden_states,
                draft_token_indices_to_sample=draft_token_indices_to_sample,
                draft_attn_metadata=draft_attn_metadata,
                **model_kwargs,
            )
        except Exception as e:
            self._unified_graph_fallback_count = (
                getattr(self, "_unified_graph_fallback_count", 0) + 1
            )
            logger.warning(
                "Unified forward failed (fallback #%d): %s",
                self._unified_graph_fallback_count,
                e,
            )
            # ``_model_forward`` already calls ``update_full_graph_params``
            # internally when running in FULL mode, so wrap its result in a
            # ``UnifiedGraphOutput`` and short-circuit the post-call update
            # below to avoid double-updating the graph params.
            return UnifiedGraphOutput(
                hidden_states=self._model_forward(
                    num_tokens_padded, input_ids, positions, intermediate_tensors, inputs_embeds, **model_kwargs
                ),
