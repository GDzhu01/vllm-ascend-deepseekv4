...
def dsa_forward(
    ...
    if use_dual and self.compress_ratio == 4 and (has_decode or has_prefill):
        ...
        if has_decode:
            (q, cos, sin, compress_cos, compress_sin,
             actual_seq_lengths_query, q_idx, kv_idx,
             ik, isc, isc_meta, wp,
             _qr_pertoken_scale) = decode_result
        ...
        if has_prefill:
            (pq, pcos, psin, pcompress_cos, pcompress_sin,
             pactual_seq_lengths_query, pq_idx, pkv_idx,
             pik, pisc, pisc_meta, pwp,
             _pqr_pertoken_scale) = prefill_result
