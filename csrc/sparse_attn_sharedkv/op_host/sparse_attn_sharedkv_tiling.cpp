...
{
    OPS_ERR_IF(opParamInfo_.q.shape == nullptr, OPS_LOG_E(opName_, "Shape of tensor q is nullptr"),
...
ge::graphStatus SASInfoParser::GetNpuInfo()
{
    ...
    OPS_ERR_IF(aicNum_ == 0 || aivNum_ == 0, OPS_LOG_E(opName_, "num of core obtained is 0."), return ge::GRAPH_FAILED);
...
ge::graphStatus SASInfoParser::GetSASTemplateMode(SASTilingInfo &sasInfo)
{
    if (opParamInfo_.oriKv.desc != nullptr) {
        ...
        OPS_LOG_E(opName_, "oriKv is nullptr");
        return ge::GRAPH_FAILED;
    }
}
...
ge::graphStatus SASInfoParser::GetMaxBlockNumPerBatch()
{
    if (opParamInfo_.oriBlockTable.tensor == nullptr) {
        OPS_LOG_E(opName_, "the layout_kv is %s, blockTable must be provided.", SASLayoutToSerialString(kvLayout_).c_str());
        return ge::GRAPH_FAILED;
    }
...
{
    OPS_ERR_IF(context == nullptr, OPS_LOG_E("SparseAttnSharedkv", "Tiling context is null."),
               return ge::GRAPH_FAILED);
