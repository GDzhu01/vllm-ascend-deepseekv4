    ...
    if [[ "${EXAMPLE_MODE}" == "eager" ]]; then
        ...
        for file in "${files[@]}"; do
            ...
            if [[ "${PKG_MODE}" == "" ]]; then
                ...
                export CUST_LIBRARY_PATH="${ASCEND_OPP_PATH}/vendors/${VENDOR_NAME}/op_api/lib"     # 仅自定义算子需要
                export CUST_INCLUDE_PATH="${ASCEND_OPP_PATH}/vendors/${VENDOR_NAME}/op_api/include" # 仅自定义算子需要
                if [[ -n "${ASCEND_CUSTOM_OPP_PATH}" ]]; then
                    CUST_VENDORS_PATH=$(dirname "${ASCEND_CUSTOM_OPP_PATH%%:*}")
                    CUST_LIBRARY_PATH="${CUST_VENDORS_PATH}/${VENDOR_NAME}/op_api/lib"
                    CUST_INCLUDE_PATH="${CUST_VENDORS_PATH}/${VENDOR_NAME}/op_api/include"
                fi
