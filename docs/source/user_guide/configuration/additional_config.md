...
| `enable_unified_mtp_graph`          | bool | `True`  | When ``speculative_config.method == "mtp"`` and ``cudagraph_mode`` is FULL or FULL_DECODE_ONLY, capture the main model + MTP draft model in a single ACLGraph so they share one NPU stream and skip the inter-graph synchronization of the legacy two-graph path. Falls back to the legacy path automatically when conflict features (async scheduling, PCP/DCP > 1, ``disable_padded_drafter_batch``, multimodal encoders, non-last PP rank, etc.) are detected. |
